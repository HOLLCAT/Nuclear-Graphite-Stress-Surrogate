#!/usr/bin/env python3
"""Run one recoverable PySR segment for a V10 signed residual stage."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--stage", choices=["geometry", "physical"], required=True)
...
def variable_complexities(feature_names: list[str]) -> list[int]:
    """Charge derived interactions more than direct measured predictors."""

    result = []
    for name in feature_names:
        if "interaction" in name or "corner" in name:
            result.append(2)
        elif name.endswith("_within_case_z"):
            result.append(2)
        else:
            result.append(1)
    return result
...
def main() -> None:
    ...
    if feature_names != scaling["features"]:
        raise ValueError("Feature names and V10 scaling file are inconsistent")
    X_scaled, y_scaled = _scale_arrays(X, y, scaling)

    # Signed residuals require operators that can express both correction
    # directions. The V9 square operator is deliberately excluded.
    binary_operators = ["+", "-", "*", "/"]
    ...
    operator_complexity = {"/": 3, "abs": 3, "tanh": 4, "gauss": 4}
    extra_sympy_mappings = {
        "gauss": lambda value: sp.exp(-(value ** 2)),
    }
