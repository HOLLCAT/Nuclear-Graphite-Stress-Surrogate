#!/usr/bin/env python3
"""Run one recoverable PySR segment for the V13 structural residual."""
...
def variable_complexities(feature_names: list[str]) -> list[int]:
    ...
    for name in feature_names:
        if "_by_" in name:
            result.append(2)
        elif name.startswith("v11_") or name.startswith("v10_"):
            result.append(2)
        elif "interaction" in name or "harmonic" in name:
            result.append(2)
...
def main() -> None:
    ...
    if feature_names != scaling["features"]:
        raise ValueError("Feature names and V13 scaling file are inconsistent")
    if len(X) != len(y) or len(X) != len(weights):
        raise ValueError("V13 feature, target and weight arrays are not row-aligned")
    ...
    if args.resume:
        ...
        if not checkpoint.exists():
            raise FileNotFoundError(f"Cannot resume V13 without {checkpoint}")
        ...
        if run_directory.exists():
            raise RuntimeError(f"Fresh V13 state already exists: {run_directory}")
        model = PySRRegressor(
    ...
    atomic_json(status_path, {
        ...
        "loss": "HuberLoss(1.0) with V13 case-balanced tail weights",
    ...
    try:
        ...
        frontier = _formula_exports(
            ...
            scaling,
            args.run_id,
            "v13_centered_structural_tail_residual",
        )
