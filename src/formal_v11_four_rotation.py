...
def register_completed_iteration_one(package_root: Path) -> dict:
    ...
    existing_path = out / "pipeline_complete.json"
    if existing_path.exists():
        existing = _load_json(existing_path)
        expected = {"status": "complete", "iteration": 1, "selected_gain": selected_gain,
                    "train_cases": 119, "validation_cases": 15, "internal_test_cases": 15,
                    "final_test_cases_read": 0}
        if any(existing.get(key) != value for key, value in expected.items()):
            raise RuntimeError("Existing Iteration-1 registration is inconsistent")
        prior = pd.read_csv(out / "registered_artifact_hashes.csv").set_index("artifact")
        for name, path in paths.items():
            if name not in prior.index or prior.loc[name, "sha256"] != _sha256(path):
                raise RuntimeError(f"Registered artifact changed: {name}")
        return existing
