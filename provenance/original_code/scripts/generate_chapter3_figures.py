...
1. The research-design workflow used in Section 3.1.
...
RAW_DATA_DIR = PROJECT_ROOT / " Case Test Data" / "FE_Results_Cases_All"
...
def plot_research_workflow(output_dir: Path) -> None:
    """Plot the case-level research design and locked evaluation path."""
    fig, axis = plt.subplots(figsize=(12.0, 7.2))
    axis.set_xlim(0, 1)
    axis.set_ylim(0, 1)
    axis.axis("off")
    ...
    add_box(
        ...
        0.32,
        0.10,
        "FE dataset\n199 cases; 400,360 elements per case",
        BLUE_LIGHT,
    )
    add_box(
        axis,
        ...
        0.32,
        0.10,
        "Data checks and preparation\ninventory, QC and predictor construction",
        BLUE_LIGHT,
    )
    add_box(
        axis,
    ...
    add_box(
        ...
        0.30,
        0.13,
        "Four case-level rotations\n119 train / 15 validation / 15 internal test\nBaseline ML and symbolic regression",
        BLUE_LIGHT,
        fontsize=9.5,
    )
    add_box(
    ...
    add_box(
        ...
        0.26,
        0.13,
        "One-time final evaluation\ncase, field and high-stress metrics",
        GREY_LIGHT,
        edgecolor=GREY,
        fontsize=9.5,
    )

    add_arrow(axis, (0.50, 0.84), (0.50, 0.77))
    add_arrow(axis, (0.50, 0.67), (0.50, 0.60))
    add_arrow(axis, (0.43, 0.50), (0.20, 0.41), "arc3,rad=0.06")
    add_arrow(axis, (0.57, 0.50), (0.80, 0.41), "arc3,rad=-0.06")
    add_arrow(axis, (0.20, 0.31), (0.19, 0.21))
    add_arrow(axis, (0.34, 0.145), (0.39, 0.145))
    add_arrow(axis, (0.64, 0.145), (0.70, 0.145))
    add_arrow(axis, (0.80, 0.31), (0.83, 0.21))

    axis.text(
        0.50,
        0.025,
        "The 50 locked cases were not used for model fitting or selection. Final evaluation followed model freezing.",
        ha="center",
        va="center",
        fontsize=9,
        color=GREY,
    ...
    axis.set_title(
        "Research design for surrogate development and evaluation",
        fontsize=14,
        pad=14,
        weight="semibold",
    )
    save_figure(fig, output_dir, "figure_3_1_research_design_workflow")
...
def main() -> None:
    args = parse_args()
    configure_style()
    plot_research_workflow(args.output_dir)
